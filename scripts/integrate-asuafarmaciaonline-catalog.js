#!/usr/bin/env node
/**
 * GirlMath — Integrate asuafarmaciaonline.pt catalog into seed
 * ============================================================
 *
 * asuafarmaciaonline.pt NÃO traz EAN no data-layer (vazio); o GTIN-13 vem do
 ...
 *
 * Uso:
 *   node scripts/integrate-asuafarmaciaonline-catalog.js [--dry-run] [--max=N] [--fuzzy=0.6]
 */
...
const ASUA_FULL = path.join(ROOT, 'data', 'catalog', 'asuafarmaciaonline-full.json');
...
const STORE_SLUG = 'asuafarmaciaonline';
...
(function main() {
  if (!fs.existsSync(ASUA_FULL)) {
    console.error('✗ Não existe', ASUA_FULL);
    console.error('  Corre primeiro: node scripts/scrape-asuafarmaciaonline-catalog.js');
    process.exit(1);
  }
  const asuaData = loadJSON(ASUA_FULL);
  const seed = loadJSON(SEED_BUNDLE);
  if (!asuaData?.products || !seed?.products) { console.error('✗ Estrutura inválida.'); process.exit(1); }

  let asua = asuaData.products.filter(p => p.status === 'ok' && p.price > 0 && p.name);
  console.log(`📦 asuafarmaciaonline: ${asuaData.products.length} entradas · ${asua.length} com nome+preço (${asua.filter(p => isRealEan(p.ean)).length} c/ EAN mpn)`);
  console.log(`📦 Seed actual:        ${seed.products.length} produtos, ${seed.stores.length} lojas · fuzzy_min=${FUZZY_MIN}\n`);

  asua = asua.slice(0, MAX_PRODUCTS);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    seed.stores.push({
      slug: STORE_SLUG,
      name: (def && def.nome) || 'A Sua Farmácia Online',
      base_url: (def && def.url) || 'https://www.asuafarmaciaonline.pt',
    ...
    console.log(`🏬 Loja "A Sua Farmácia Online" registada em seed.stores[].`);
  }
  ...
  for (const ep of asua) {
    ...
    // Se o produto-alvo tinha EAN sintético e a asua traz EAN real (mpn), faz upgrade.
    ...
    const r = upsertStoreItem(
      ...
      target.ean, ep, asuaData.scraped_at
    );
  ...
  console.log(`  Ofertas asuafarmaciaonline:  +${added} adicionadas, ${updated} actualizadas (total ${sp.items.length})`);
  if (fuzzySamples.length) {
    ...
    for (const s of toShow) { console.log(`    [${s.j.toFixed(2)}] asua: ${s.a}`); console.log(`          seed: ${s.s}`); }
  }
