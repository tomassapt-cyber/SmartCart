#!/usr/bin/env node
/**
 * CosMath — Integrate aveirofarma.pt catalog into seed
 * ============================================================
 * Aveiro Farma (OpenCart PT) expõe gtin14 REAL no JSON-LD → matching FORTE
 * por EAN, como cocooncenter / mycosmetics.
 *
 ...
 *     não-cosmético; catálogo é maioria suplementos, já filtrados no scrape).
 *
 * NUNCA altera nomes de produtos existentes. Idempotente.
 *
 * Uso:
 *   node scripts/integrate-aveirofarma-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'aveirofarma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'aveirofarma';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-aveirofarma-catalog.js'); process.exit(1); }
  ...
  let av = data.products.filter(p => p.status === 'ok' && isRealEan(p.ean) && p.price > 0).slice(0, MAX);
  console.log(`📦 aveirofarma: ${data.products.length} entradas · ${av.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Aveiro Farma',
      base_url: 'https://aveirofarma.pt', logo_url: null,
      free_shipping_threshold: 30,
      shipping_zones: { mainland: 3.90, madeira: 6.90, acores: 6.90 },
    });
    console.log('🏬 Loja "Aveiro Farma" registada em seed.stores[].');
  }
  ...
  for (const ep of av) {
    ...
    if (!target) {
      ...
      target = { ean: ep.ean, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'aveirofarma-catalog' };
  ...
  console.log('══════ Resumo (aveirofarma) ══════');
  ...
  console.log(`  Ofertas aveirofarma:       +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração aveirofarma completa.');
})();
