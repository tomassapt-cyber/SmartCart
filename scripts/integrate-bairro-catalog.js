#!/usr/bin/env node
/**
 * GirlMath — Integrate Bairro da Saúde catalog into seed
 * ============================================================
 *
 * Lê data/catalog/bairro-saude-full.json e funde no seed-bundle:
 *  1. Match por fingerprint (brand+name canónico — multilang).
 *  2. Se não houver match, ADICIONA como novo produto com EAN sintético.
 *
 * Bairro da Saúde é Shopify → não expõe `barcode` na API pública. Tudo o
 * matching é por fingerprint. O `sku` Shopify é apenas referência
 * interna da loja, não GTIN.
 *
 * Em ambos os casos, faz upsert da oferta Bairro da Saúde como store_product
 ...
 *
 * Uso:
 *   node scripts/integrate-bairro-saude-catalog.js
 *   node scripts/integrate-bairro-saude-catalog.js --dry-run
 *   node scripts/integrate-bairro-saude-catalog.js --max=200
 */
...
const BAIRRO_FULL = path.join(ROOT, 'data', 'catalog', 'bairro-saude-full.json');
...
function syntheticEan(p) {
  ...
  return `bairro-saude-${slug}`;
}

(function main() {
  if (!fs.existsSync(BAIRRO_FULL)) {
    console.error('✗ Não existe', BAIRRO_FULL);
    console.error('  Corre primeiro: node scripts/scrape-bairro-saude-catalog.js');
    process.exit(1);
  }
  ...
  const efData = loadJSON(BAIRRO_FULL);
  ...
  console.log(`📦 Bairro da Saúde catalog: ${efData.products.length} produtos`);
  ...
  // Fingerprint: principal mecanismo de match para Bairro da Saúde.
  ...
  // Limpar ofertas Bairro da Saúde com preço "fake" (placeholder do build-catalog
  ...
  // Estratégia conservadora: SÓ remover ofertas Bairro da Saúde cujos EANs vão
  ...
  for (const ep of efToIntegrate) {
    ...
    // o mesmo SKU. Sem barcode/EAN público no Shopify (Bairro da Saúde),
    ...
    // ── Upsert da oferta Bairro da Saúde com variant merge ──
    // upsertStoreItem trata da união de variantes: se já existe um item
    // Bairro da Saúde para este EAN, mantém variantes antigas + acrescenta novas.
    // Para Bairro da Saúde o "produto" do Shopify geralmente é 1 variante (volume).
  ...
  // Stats — ofertas Bairro da Saúde agora vs antes
  console.log(`\n  Ofertas Bairro da Saúde totais: ${efSp.items.length}`);
  ...
  if (r.status === 0) {
    console.log('\n✅ Integração Bairro da Saúde completa.');
  }
})();
