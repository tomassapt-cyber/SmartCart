#!/usr/bin/env node
/**
 * GirlMath — Integrate barreiros.pt catalog into seed
 * ============================================================
 *
 * barreiros.pt NÃO traz EAN no data-layer (vazio); o GTIN-13 vem do
 ...
 *
 * Uso:
 *   node scripts/integrate-barreiros-catalog.js [--dry-run] [--max=N] [--fuzzy=0.6]
 */
...
const BARREIROS_FULL = path.join(ROOT, 'data', 'catalog', 'barreiros-full.json');
...
(function main() {
  if (!fs.existsSync(BARREIROS_FULL)) {
    console.error('✗ Não existe', BARREIROS_FULL);
    console.error('  Corre primeiro: node scripts/scrape-barreiros-catalog.js');
    process.exit(1);
  }
  const barrData = loadJSON(BARREIROS_FULL);
  const seed = loadJSON(SEED_BUNDLE);
  if (!barrData?.products || !seed?.products) { console.error('✗ Estrutura inválida.'); process.exit(1); }

  let barr = barrData.products.filter(p => p.status === 'ok' && p.price > 0 && p.name);
  console.log(`📦 barreiros: ${barrData.products.length} entradas · ${barr.length} com nome+preço (${barr.filter(p => isRealEan(p.ean)).length} c/ EAN mpn)`);
  console.log(`📦 Seed actual:        ${seed.products.length} produtos, ${seed.stores.length} lojas · fuzzy_min=${FUZZY_MIN}\n`);

  barr = barr.slice(0, MAX_PRODUCTS);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    seed.stores.push({
      slug: STORE_SLUG,
      name: (def && def.nome) || 'Farmácia Barreiros',
      base_url: (def && def.url) || 'https://www.barreiros.pt',
    ...
    console.log(`🏬 Loja "Farmácia Barreiros" registada em seed.stores[].`);
  }
  ...
  for (const ep of barr) {
    ...
    // Se o produto-alvo tinha EAN sintético e a barr traz EAN real (mpn), faz upgrade.
    ...
    const r = upsertStoreItem(
      ...
      target.ean, ep, barrData.scraped_at
    );
    if (r.action === 'added') added++; else if (r.action === 'merged') updated++;
  }

  console.log('══════ Resumo da integração (barreiros) ══════');
  ...
  console.log(`  Ofertas barreiros:  +${added} adicionadas, ${updated} actualizadas (total ${sp.items.length})`);
  if (fuzzySamples.length) {
    ...
    for (const s of toShow) { console.log(`    [${s.j.toFixed(2)}] barr: ${s.a}`); console.log(`          seed: ${s.s}`); }
  }
