#!/usr/bin/env node
/**
 * CosMath — Integrate beleza37 catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/beleza37-full.json (Jumpseller; JSON-LD sku=CNP/EAN)
 * e funde no seed em MODO COMPARAÇÃO: beleza37 entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-beleza37-catalog.js
 *   node scripts/integrate-beleza37-catalog.js --dry-run
 *   node scripts/integrate-beleza37-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'beleza37-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'beleza37';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-beleza37-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 beleza37: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo beleza37. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(beleza37)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'Beleza37',
      base_url: 'https://www.beleza37.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "beleza37" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (beleza37 · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas beleza37:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
