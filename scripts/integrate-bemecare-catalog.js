#!/usr/bin/env node
/**
 * CosMath — Integrate bemecare (ver scraper) catalog into seed
 * ============================================================
 * BeMeCare expõe CNP (sku) sempre + EAN pontual — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-bemecare-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'bemecare-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'bemecare';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-bemecare-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 bemecare: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'BeMeCare',
      base_url: 'https://bemecare.com', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "BeMeCare" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('bemecare-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'bemecare-catalog' };
  ...
  console.log(`  Ofertas bemecare:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
