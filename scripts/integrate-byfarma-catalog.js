#!/usr/bin/env node
/**
 * GirlMath — Integrate byFarma catalog into seed
 * ============================================================
 *
 * Lê data/catalog/byfarma-full.json e funde no seed-bundle:
 *  1. Match por fingerprint (brand+name canónico — multilang).
 *  2. Se não houver match, ADICIONA como novo produto com EAN sintético.
 *
 * byFarma é Shopify → não expõe `barcode` na API pública. Tudo o
 * matching é por fingerprint. O `sku` Shopify é apenas referência
 * interna da loja, não GTIN.
 *
 * Em ambos os casos, faz upsert da oferta byFarma como store_product
 ...
 *
 * Uso:
 *   node scripts/integrate-byfarma-catalog.js
 *   node scripts/integrate-byfarma-catalog.js --dry-run
 *   node scripts/integrate-byfarma-catalog.js --max=200
 */
...
function syntheticEan(p) {
  // EAN sintético derivado do URL slug (byFarma não tem 'handle', usa URL).
...
(function main() {
  if (!fs.existsSync(BYFARMA_FULL)) {
    console.error('✗ Não existe', BYFARMA_FULL);
    console.error('  Corre primeiro: node scripts/scrape-byfarma-catalog.js');
    process.exit(1);
  }
  ...
  const efData = loadJSON(BYFARMA_FULL);
  ...
  console.log(`📦 byFarma catalog: ${efData.products.length} produtos`);
  ...
  // Fingerprint: principal mecanismo de match para byFarma.
  ...
  // Limpar ofertas byFarma com preço "fake" (placeholder do build-catalog
  ...
  // Estratégia conservadora: SÓ remover ofertas byFarma cujos EANs vão
  ...
  for (const ep of efToIntegrate) {
    ...
    if (!targetProduct) {
      ...
      if (fp && fpIndex[fp]) {
        ...
        matchedByFp++;
        // Side-effect: se target tem EAN sintético e byFarma tem EAN real,
    ...
    // o mesmo SKU. Sem barcode/EAN público no Shopify (byFarma),
    ...
    // ── Upsert da oferta byFarma com variant merge ──
    // upsertStoreItem trata da união de variantes: se já existe um item
    // byFarma para este EAN, mantém variantes antigas + acrescenta novas.
    // Para byFarma o "produto" do Shopify geralmente é 1 variante (volume).
  ...
  console.log(`  byFarma store_products:               +${storeProductsAdded} adicionados, ${storeProductsUpdated} actualizados`);
  console.log('');
  ...
  // Stats — ofertas byFarma agora vs antes
  console.log(`\n  Ofertas byFarma totais: ${efSp.items.length}`);
