#!/usr/bin/env node
/**
 * CosMath — Integrate cocooncenter.pt catalog into seed
 * ============================================================
 * Cocooncenter (parafarmácia FR, storefront PT) expõe gtin13 REAL no JSON-LD →
 * matching FORTE por EAN, como pharma-gdd / lojadafarmacia / byFarma.
 *
 * POLÍTICA (igual à lojafarmacia, conservadora):
 ...
 *     não-cosmético: oral, suplementos, veterinária, pensos…).
 *
 * NUNCA altera nomes de produtos existentes. Idempotente.
 *
 * Uso:
 *   node scripts/integrate-cocooncenter-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'cocooncenter-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'cocooncenter';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-cocooncenter-catalog.js --dermo'); process.exit(1); }
  ...
  let cc = data.products.filter(p => p.status === 'ok' && isRealEan(p.ean) && p.price > 0).slice(0, MAX);
  console.log(`📦 cocooncenter: ${data.products.length} entradas · ${cc.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Cocooncenter',
      base_url: 'https://www.cocooncenter.pt', logo_url: null,
      free_shipping_threshold: 59,
      shipping_zones: { mainland: 4.95, madeira: 4.95, acores: 4.95 },
    });
    console.log('🏬 Loja "Cocooncenter" registada em seed.stores[].');
  }
  ...
  for (const ep of cc) {
    ...
    if (!target) {
      ...
      target = { ean: ep.ean, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'cocooncenter-catalog' };
      seed.products.push(target); eanIndex[ep.ean] = target;
  ...
  console.log('══════ Resumo (cocooncenter) ══════');
  ...
  console.log(`  Ofertas cocooncenter:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  // Pós-processo (mesmo pipeline da lojafarmacia, idempotente).
  ...
  console.log('\n✅ Integração cocooncenter completa.');
})();
