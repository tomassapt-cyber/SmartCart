#!/usr/bin/env node
/**
 * CosMath — Integrate cosmeticfan.com catalog into seed
 * ============================================================
 * CosmeticFan expõe EAN real (mpn) E CNP (sku) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-cosmeticfan-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'cosmeticfan-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'cosmeticfan';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-cosmeticfan-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 cosmeticfan: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'CosmeticFan',
      base_url: 'https://www.cosmeticfan (ver header)', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.99, madeira: 3.99, acores: 3.99 },
    });
    console.log('🏬 Loja "CosmeticFan" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('cosmeticfan-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'cosmeticfan-catalog' };
  ...
  console.log('══════ Resumo (cosmeticfan) ══════');
  ...
  console.log(`  Ofertas cosmeticfan:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração cosmeticfan completa.');
})();
