#!/usr/bin/env node
/**
 * CosMath — Integrate DocMorris catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/docmorris-full.json (PDPs JSON-LD sem gtin; MODO COMPARAÇÃO) e funde no seed.
 *
 ...
 *
 * Uso:
 *   node scripts/integrate-docmorris-catalog.js
 *   node scripts/integrate-docmorris-catalog.js --dry-run
 *   node scripts/integrate-docmorris-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'docmorris-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'docmorris';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-docmorris-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 docmorris: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'DocMorris',
      base_url: 'https://www.docmorris.pt',
      logo_url: null,
      // TODO confirmar portes/threshold reais na sheet
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.99, madeira: 3.99, acores: 3.99 },
    });
    console.log(`🏬 Loja "DocMorris" registada em seed.stores[].`);
  }
  ...
  console.log(`  Ofertas docmorris:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
