#!/usr/bin/env node
/**
 * CosMath — Integrate farmacentral catalog into seed
 * ============================================================
 * Loja 72 (2026-07-30). Farmácia PT na mesma plataforma do dermis/bemecare —
 * ver scripts/scrape-farmacentral-catalog.js para as TRÊS diferenças de campos
 * que obrigaram a um scraper próprio em vez de reaproveitar o do dermis.
 *
 * CHAVE: **CNP em 100% dos produtos, EAN em apenas 60%.** É o inverso do
 * dermis (81% de EAN). Aqui o CNP é a chave principal: os produtos sem EAN
 * entram com EAN sintético "farmacentral-<slug>" e o apply-cnp-merge.js
 * resgata-os depois para o EAN real quando outra loja partilhar o mesmo CNP.
 * É por isso que este integrador NÃO deve ser corrido sem o apply-cnp-merge
 * a seguir (já está na cadeia, no fim do ficheiro).
 *
 * PORQUE VALE A PENA: verificado antes de integrar que NÃO é catálogo
 * espelhado do dermis — 40% de sobreposição em CNP, 29% em EAN, logo ~60% dos
 * produtos são novos no site.
 *
 ...
 *     não-cosmético).
 *
 * ⚠️ PORTES NÃO VERIFICADOS. Regista-se com os mesmos valores por omissão do
 * dermis/bemecare (49€ grátis, 3,95€). As páginas de portes da loja não estão
 * em nenhum endereço óbvio (todos 404) e o site é renderizado no cliente. NÃO
 * se acrescentou entrada em data/store-shipping.json de propósito: nesse
 * ficheiro, uma entrada AUSENTE significa "não verificado", e inventar um
 * valor lá seria afirmar uma coisa que não se confirmou. Quando alguém
 * confirmar os portes reais, é lá que se acrescenta.
 *
 * NUNCA altera nomes de produtos existentes. Idempotente.
 *
 * Uso:
 *   node scripts/integrate-farmacentral-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'farmacentral-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'farmacentral';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-farmacentral-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 farmacentral: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Farmacentral',
      base_url: 'https://farmacentral.pt', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "Farmacentral" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('farmacentral-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'farmacentral-catalog' };
  ...
  console.log('══════ Resumo (farmacentral) ══════');
  ...
  console.log(`  Ofertas farmacentral:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração farmacentral completa.');
})();
