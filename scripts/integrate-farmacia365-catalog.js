#!/usr/bin/env node
/**
 * GirlMath — Integrate Farmácia 365 catalog into seed
 * ============================================================
 *
 * Lê data/catalog/farmacia365-full.json e funde no seed-bundle:
 *  1. Match por fingerprint (brand+name canónico — multilang).
 *  2. Se não houver match, ADICIONA como novo produto com EAN sintético.
 *
 * Farmácia 365 é Shopify → não expõe `barcode` na API pública. Tudo o
 * matching é por fingerprint. O `sku` Shopify é apenas referência
 * interna da loja, não GTIN.
 *
 * Em ambos os casos, faz upsert da oferta Farmácia 365 como store_product
 ...
 *
 * Uso:
 *   node scripts/integrate-farmacia365-catalog.js
 *   node scripts/integrate-farmacia365-catalog.js --dry-run
 *   node scripts/integrate-farmacia365-catalog.js --max=200
 */
...
const FARMACIA365_FULL = path.join(ROOT, 'data', 'catalog', 'farmacia365-full.json');
...
function syntheticEan(p) {
  // EAN sintético derivado do URL slug (Farmácia 365 não tem 'handle', usa URL).
  // Estável entre runs — URL é o ID único do produto.
  const url = p.url || '';
  const slug = url.split('/').pop().slice(0, 50).replace(/[^a-z0-9-]/g, '');
  return `farmacia365-${slug}`;
}

(function main() {
  if (!fs.existsSync(FARMACIA365_FULL)) {
    console.error('✗ Não existe', FARMACIA365_FULL);
    console.error('  Corre primeiro: node scripts/scrape-farmacia365-catalog.js');
    process.exit(1);
  }
  ...
  const efData = loadJSON(FARMACIA365_FULL);
  ...
  console.log(`📦 Farmácia 365 catalog: ${efData.products.length} produtos`);
  ...
  // Fingerprint: principal mecanismo de match para Farmácia 365.
  ...
  // Limpar ofertas Farmácia 365 com preço "fake" (placeholder do build-catalog
  ...
  // Estratégia conservadora: SÓ remover ofertas Farmácia 365 cujos EANs vão
  ...
  // Farmácia 365 expõe GTIN-13 real → matching por EAN funciona forte (cross-store)
  let matchedByEan = 0;
  let upgradedFromSynthetic = 0;

  for (const ep of efToIntegrate) {
    // ── 0. Match por EAN real (preferencial — Farmácia 365 dá GTIN-13) ──
    ...
    if (!targetProduct) {
      ...
      if (fp && fpIndex[fp]) {
        ...
        matchedByFp++;
        // Side-effect: se target tem EAN sintético e Farmácia 365 tem EAN real,
    ...
    // o mesmo SKU. Sem barcode/EAN público no Shopify (Farmácia 365),
    ...
    // ── 3. Não match → criar como novo (preferir EAN real Farmácia 365) ──
    if (!targetProduct) {
    ...
    // ── Upsert da oferta Farmácia 365 com variant merge ──
    // upsertStoreItem trata da união de variantes: se já existe um item
    // Farmácia 365 para este EAN, mantém variantes antigas + acrescenta novas.
    // Para Farmácia 365 o "produto" do Shopify geralmente é 1 variante (volume).
  ...
  console.log(`  Farmácia 365 store_products:               +${storeProductsAdded} adicionados, ${storeProductsUpdated} actualizados`);
  console.log('');
  ...
  // Stats — ofertas Farmácia 365 agora vs antes
  console.log(`\n  Ofertas Farmácia 365 totais: ${efSp.items.length}`);
  ...
  if (r.status === 0) {
    console.log('\n✅ Integração Farmácia 365 completa.');
  }
})();
