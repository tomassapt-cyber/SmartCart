#!/usr/bin/env node
/**
 * CosMath — Integrate farmaciagarcia (ver scraper) catalog into seed
 * ============================================================
 * Farmácia Garcia expõe CNP (sku) sempre; EAN ausente — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-farmaciagarcia-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'farmaciagarcia-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'farmaciagarcia';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-farmaciagarcia-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 farmaciagarcia: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Farmácia Garcia',
      base_url: 'https://loja.farmaciagarcia.net', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "Farmácia Garcia" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('farmaciagarcia-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'farmaciagarcia-catalog' };
  ...
  console.log('══════ Resumo (farmaciagarcia) ══════');
  ...
  console.log(`  Ofertas farmaciagarcia:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração farmaciagarcia completa.');
})();
