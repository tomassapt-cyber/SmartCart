#!/usr/bin/env node
/**
 * CosMath — Integrate farmaciaideal.pt catalog into seed
 * ============================================================
 * Farmácia Ideal expõe EAN real (mpn) E CNP (sku) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-farmaciaideal-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'farmaciaideal-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'farmaciaideal';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-farmaciaideal-catalog.js'); process.exit(1); }
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Farmácia Ideal',
      base_url: 'https://www.farmaciaideal.pt', logo_url: null,
      free_shipping_threshold: 39,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "Farmácia Ideal" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('farmaciaideal-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'farmaciaideal-catalog' };
  ...
  console.log('══════ Resumo (farmaciaideal) ══════');
  ...
  console.log(`  Ofertas farmaciaideal:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
