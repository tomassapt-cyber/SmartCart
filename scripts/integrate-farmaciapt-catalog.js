#!/usr/bin/env node
/**
 * GirlMath — Integrate Farmácia.pt catalog into seed
 * ============================================================
 *
 * Lê data/catalog/farmaciapt-full.json e funde no seed-bundle:
 *  1. Match por fingerprint (brand+name canónico — multilang).
 *  2. Se não houver match, ADICIONA como novo produto com EAN sintético.
 *
 * Farmácia.pt é Shopify → não expõe `barcode` na API pública. Tudo o
 * matching é por fingerprint. O `sku` Shopify é apenas referência
 * interna da loja, não GTIN.
 *
 * Em ambos os casos, faz upsert da oferta Farmácia.pt como store_product
 ...
 *   node scripts/integrate-farmaciapt-catalog.js --dry-run
 *   node scripts/integrate-farmaciapt-catalog.js --max=200
 */
...
const FARMACIAPT_FULL = path.join(ROOT, 'data', 'catalog', 'farmaciapt-full.json');
...
function syntheticEan(p) {
  // EAN sintético derivado do URL slug (Farmácia.pt não tem 'handle', usa URL).
  // Estável entre runs — URL é o ID único do produto.
  const url = p.url || '';
  const slug = url.split('/').pop().slice(0, 50).replace(/[^a-z0-9-]/g, '');
  return `farmaciapt-${slug}`;
}

(function main() {
  if (!fs.existsSync(FARMACIAPT_FULL)) {
    console.error('✗ Não existe', FARMACIAPT_FULL);
    console.error('  Corre primeiro: node scripts/scrape-farmaciapt-catalog.js');
    process.exit(1);
  }
  ...
  const efData = loadJSON(FARMACIAPT_FULL);
  ...
  console.log(`📦 Farmácia.pt catalog: ${efData.products.length} produtos`);
  ...
  const _existingUrls = new Set(((seed.store_products.find(g => g.store_slug === 'farmaciapt') || {}).items || []).map(it => it.url).filter(Boolean));
  let keptKnown = 0;
  ...
  // Fingerprint: principal mecanismo de match para Farmácia.pt.
  ...
  // Estratégia conservadora: SÓ remover ofertas Farmácia.pt cujos EANs vão
  ...
  // Farmácia.pt expõe GTIN-13 real → matching por EAN funciona forte (cross-store)
  let matchedByEan = 0;
  let upgradedFromSynthetic = 0;

  for (const ep of efToIntegrate) {
    // ── 0. Match por EAN real (preferencial — Farmácia.pt dá GTIN-13) ──
    ...
    if (!targetProduct) {
      ...
      if (fp && fpIndex[fp]) {
        ...
        matchedByFp++;
        // Side-effect: se target tem EAN sintético e Farmácia.pt tem EAN real,
    ...
    // o mesmo SKU. Sem barcode/EAN público no Shopify (Farmácia.pt),
    ...
    // ── 3. Não match → criar como novo (preferir EAN real Farmácia.pt) ──
    if (!targetProduct) {
    ...
    // ── Upsert da oferta Farmácia.pt com variant merge ──
    // upsertStoreItem trata da união de variantes: se já existe um item
    // Farmácia.pt para este EAN, mantém variantes antigas + acrescenta novas.
    // Para Farmácia.pt o "produto" do Shopify geralmente é 1 variante (volume).
  ...
  console.log(`  Farmácia.pt store_products:               +${storeProductsAdded} adicionados, ${storeProductsUpdated} actualizados`);
  console.log('');
  ...
  // Stats — ofertas Farmácia.pt agora vs antes
  console.log(`\n  Ofertas Farmácia.pt totais: ${efSp.items.length}`);
  ...
  if (r.status === 0) {
    console.log('\n✅ Integração Farmácia.pt completa.');
  }
})();
