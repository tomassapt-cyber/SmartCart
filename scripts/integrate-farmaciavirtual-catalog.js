#!/usr/bin/env node
/**
 * CosMath — Integrate farmaciavirtual.pt catalog into seed
 * ============================================================
 *
 * Farmácia Virtual (maior rede PT, Magento). O catálogo traz CNP (sku de
 ...
 *
 * Adiciona a oferta Farmácia Virtual ao produto existente. Idempotente.
 * Não corre dedup (não cria/merge produtos).
 *
 * Uso:
 *   node scripts/integrate-farmaciavirtual-catalog.js
 *   node scripts/integrate-farmaciavirtual-catalog.js --dry-run
 *   node scripts/integrate-farmaciavirtual-catalog.js --no-inject
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'farmaciavirtual-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const CATALOG_DIR = path.join(ROOT, 'data', 'catalog');
const STORE_SLUG = 'farmaciavirtual';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-farmaciavirtual-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 farmaciavirtual: ${cat.products.length} entradas · ${items.length} com preço+CNP`);
  ...
  for (const file of fs.readdirSync(CATALOG_DIR)) {
    if (!file.endsWith('-full.json') || file.startsWith('farmaciavirtual')) continue;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Farmácia Virtual',
      base_url: 'https://farmaciavirtual.pt', logo_url: null,
      free_shipping_threshold: 30,
      shipping_zones: { mainland: 3.99, madeira: 1.5, acores: 1.5 },
    });
    console.log('🏬 Loja "Farmácia Virtual" registada em seed.stores[].');
  }
  ...
  console.log('══════ Resumo (farmaciavirtual) ══════');
  ...
  if (r.status === 0) console.log('\n✅ Integração farmaciavirtual completa.');
})();
