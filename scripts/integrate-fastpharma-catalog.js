#!/usr/bin/env node
/**
 * CosMath — Integrate FastPharma catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/fastpharma-full.json (Magento; JSON-LD sku=mpn=CNP)
 * e funde no seed em MODO COMPARAÇÃO: FastPharma entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-fastpharma-catalog.js
 *   node scripts/integrate-fastpharma-catalog.js --dry-run
 *   node scripts/integrate-fastpharma-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'fastpharma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'fastpharma';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-fastpharma-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 fastpharma: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo fastpharma. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(fastpharma)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    console.log(`🏬 Loja "FastPharma" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (FastPharma · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas fastpharma:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  if (r.status === 0) console.log('\n✅ Integração FastPharma completa.');
})();
