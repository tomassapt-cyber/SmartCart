#!/usr/bin/env node
/**
 * CosMath — Integrate GO-farma catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/gofarma-full.json (Magento; JSON-LD gtin13=EAN + sku=CNP)
 * e funde no seed em MODO COMPARAÇÃO: GO-farma entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-gofarma-catalog.js
 *   node scripts/integrate-gofarma-catalog.js --dry-run
 *   node scripts/integrate-gofarma-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'gofarma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'gofarma';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-gofarma-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 gofarma: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo gofarma. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(gofarma)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'GO-farma',
      base_url: 'https://www.go-farma.com',
      logo_url: null,
    ...
    console.log(`🏬 Loja "GO-farma" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (GO-farma · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas gofarma:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
