#!/usr/bin/env node
/**
 * CosMath — Integrate Hiper Farma catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/hiperfarma-full.json (Shopify products.json; sku=CNP/EAN)
 * e funde no seed em MODO COMPARAÇÃO: Hiper Farma entra SÓ como fonte de preço
 ...
 * por CNP, zero risco de poluir. Mesmo padrão da a-tua-farmacia.
 *
 ...
 *
 * Uso:
 *   node scripts/integrate-hiperfarma-catalog.js
 *   node scripts/integrate-hiperfarma-catalog.js --dry-run
 *   node scripts/integrate-hiperfarma-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'hiperfarma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'hiperfarma';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-hiperfarma-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 hiperfarma: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo hiperfarma. É o sinal forte:
  ...
  function resolveBrand(item) {
    // ⚠ O vendor da hiperfarma é o DISTRIBUIDOR ("Laboratoires Uriage", "IFC",
    // "Pierre Fabre Dermo-Cosmétique", "Cosveritas", "Hiper Farma"), não a
    // marca de consumo → só o usamos se for uma marca CONHECIDA do seed;
    // senão resolvemos pelo prefixo do título ("Uriage Água Termal…"→Uriage).
    const v = (item.brand || '').trim();
    if (v && brandSet.has(norm(v))) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'Hiper Farma',
      base_url: 'https://www.hiperfarma.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "Hiper Farma" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (Hiper Farma · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas hiperfarma:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  if (r.status === 0) console.log('\n✅ Integração Hiper Farma completa.');
})();
