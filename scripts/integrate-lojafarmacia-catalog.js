#!/usr/bin/env node
/**
 * GirlMath — Integrate lojadafarmacia.com catalog into seed
 * ============================================================
 *
 * Lê data/catalog/lojafarmacia-full.json e funde no seed-bundle.
 *
 * lojadafarmacia.com expõe GTIN-13 REAL (gtin13 do JSON-LD) → matching forte
 * por EAN, tal como Druni/byFarma/Farmácia.pt/afarmaciaonline.
 *
 ...
 *   • SEM match → IGNORA (NÃO cria produto novo). O catálogo não traz
 *     categoria fiável; criar produtos sem categoria poluiria os filtros.
 *     v1 adiciona só a loja-farmacia como FONTE DE PREÇO sobre produtos
 *     que já temos. (Breadth de produtos novos fica para v2.)
 *
 * NUNCA altera nomes de produtos (fingerprint do dedup). Idempotente.
 *
 * Uso:
 *   node scripts/integrate-lojafarmacia-catalog.js
 *   node scripts/integrate-lojafarmacia-catalog.js --dry-run
 *   node scripts/integrate-lojafarmacia-catalog.js --max=200
 */
...
const STORE_SLUG = 'loja-farmacia';
...
(function main() {
  if (!fs.existsSync(LDF_FULL)) {
    console.error('✗ Não existe', LDF_FULL);
    console.error('  Corre primeiro: node scripts/scrape-lojafarmacia-catalog.js');
    process.exit(1);
  }
  const ldfData = loadJSON(LDF_FULL);
  const seed = loadJSON(SEED_BUNDLE);
  if (!ldfData?.products || !seed?.products) {
    console.error('✗ Ficheiros com estrutura inválida.');
    process.exit(1);
  }

  let ldf = ldfData.products.filter(p => p.status === 'ok' && isRealEan(p.ean) && p.price > 0);
  console.log(`📦 lojadafarmacia catalog: ${ldfData.products.length} entradas · ${ldf.length} com EAN real + preço`);
  console.log(`📦 Seed actual:            ${seed.products.length} produtos, ${seed.stores.length} lojas\n`);

  ldf = ldf.slice(0, MAX_PRODUCTS);
  ...
  // Grupo de store_products da loja-farmacia
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    const storeEntry = {
      slug: STORE_SLUG,
      name: (def && def.nome) || 'Loja da Farmácia',
      base_url: (def && def.url) || 'https://www.lojadafarmacia.com',
  ...
  for (const ep of ldf) {
    ...
    const r = upsertStoreItem(
      ...
      target.ean, ep, ldfData.scraped_at
    );
  ...
  if (r.status === 0) console.log('\n✅ Integração loja-farmacia completa.');
})();
