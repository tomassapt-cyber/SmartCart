#!/usr/bin/env node
/**
 * CosMath — Integrate mycosmetics.pt catalog into seed
 * ============================================================
 * My Cosmetics (Shopify PT) expõe gtin13 REAL no JSON-LD → matching FORTE
 * por EAN, como cocooncenter / pharma-gdd.
 *
 ...
 *
 * Uso:
 *   node scripts/integrate-mycosmetics-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'mycosmetics-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'mycosmetics';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-mycosmetics-catalog.js'); process.exit(1); }
  ...
  let mc = data.products.filter(p => p.status === 'ok' && isRealEan(p.ean) && p.price > 0).slice(0, MAX);
  console.log(`📦 mycosmetics: ${data.products.length} entradas · ${mc.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'My Cosmetics',
      base_url: 'https://www.mycosmetics.pt', logo_url: null,
      free_shipping_threshold: 40,
      shipping_zones: { mainland: 3.99, madeira: 5.99, acores: 5.99 },
    });
    console.log('🏬 Loja "My Cosmetics" registada em seed.stores[].');
  }
  ...
  for (const ep of mc) {
    ...
    if (!target) {
      ...
      target = { ean: ep.ean, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'mycosmetics-catalog' };
  ...
  console.log('══════ Resumo (mycosmetics) ══════');
  ...
  console.log(`  Ofertas mycosmetics:       +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração mycosmetics completa.');
})();
