#!/usr/bin/env node
/**
 * CosMath — Integrate notino.pt catalog into seed
 * ============================================================
 * Notino (maior marketplace de beleza PT) expõe gtin13 REAL no JSON-LD →
 ...
 *
 * Uso:
 *   node scripts/integrate-notino-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'notino-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'notino';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-notino-catalog.js '); process.exit(1); }
  ...
  console.log(`📦 notino: ${data.products.length} entradas · ${cc.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Notino',
      base_url: 'https://www.notino.pt', logo_url: null,
      free_shipping_threshold: 25,
      shipping_zones: { mainland: 3.49, madeira: 3.49, acores: 3.49 },
    });
    console.log('🏬 Loja "Notino" registada em seed.stores[].');
  }
  ...
  for (const ep of cc) {
    ...
    if (!target) {
      ...
      target = { ean: ep.ean, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'notino-catalog' };
  ...
  console.log(`  Ofertas notino:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
