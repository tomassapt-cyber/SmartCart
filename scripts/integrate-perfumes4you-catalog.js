#!/usr/bin/env node
/**
 * CosMath — Integrate Perfumes4you catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/perfumes4you-full.json (WooCommerce; JSON-LD sku=EAN-13 real)
 * e funde no seed em MODO COMPARAÇÃO: Perfumes4you entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-perfumes4you-catalog.js
 *   node scripts/integrate-perfumes4you-catalog.js --dry-run
 *   node scripts/integrate-perfumes4you-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'perfumes4you-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'perfumes4you';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-perfumes4you-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 perfumes4you: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo perfumes4you. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(perfumes4you)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'Perfumes4you',
      base_url: 'https://perfumes4you.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "Perfumes4you" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (Perfumes4you · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas perfumes4you:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  if (r.status === 0) console.log('\n✅ Integração Perfumes4you completa.');
})();
