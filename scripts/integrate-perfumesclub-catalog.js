#!/usr/bin/env node
/**
 * CosMath — Integrate Perfume's Club catalog into seed
 * ============================================================
 * Perfume's Club (perfumaria ES, storefront PT perfumesclub.pt): JSON-LD com gtin13 REAL -> matching forte por EAN.
 *
 ...
 *
 * Uso:
 *   node scripts/integrate-perfumesclub-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'perfumesclub-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'perfumesclub';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-perfumesclub-catalog.js --dermo'); process.exit(1); }
  ...
  console.log(`📦 perfumesclub: ${data.products.length} entradas · ${cc.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: "Perfume's Club",
      base_url: 'https://www.perfumesclub.pt', logo_url: null,
    ...
    console.log('🏬 Loja Perfumes Club registada em seed.stores[].');
  }
  ...
  console.log('══════ Resumo (perfumesclub) ══════');
  ...
  console.log(`  Ofertas perfumesclub:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração perfumesclub completa.');
})();
