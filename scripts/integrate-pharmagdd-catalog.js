#!/usr/bin/env node
/**
 * GirlMath — Integrate pharma-gdd.pt catalog into seed
 * ============================================================
 *
 * pharma-gdd.pt NÃO traz EAN no data-layer (vazio); o GTIN-13 vem do
 ...
 *
 * Uso:
 *   node scripts/integrate-pharma-gdd-catalog.js [--dry-run] [--max=N] [--fuzzy=0.6]
 */
...
const PGDD_FULL = path.join(ROOT, 'data', 'catalog', 'pharmagdd-full.json');
...
(function main() {
  if (!fs.existsSync(PGDD_FULL)) {
    console.error('✗ Não existe', PGDD_FULL);
    console.error('  Corre primeiro: node scripts/scrape-pharma-gdd-catalog.js');
    process.exit(1);
  }
  const pgddData = loadJSON(PGDD_FULL);
  const seed = loadJSON(SEED_BUNDLE);
  if (!pgddData?.products || !seed?.products) { console.error('✗ Estrutura inválida.'); process.exit(1); }

  let pgdd = pgddData.products.filter(p => p.status === 'ok' && p.price > 0 && p.name);
  console.log(`📦 pharma-gdd: ${pgddData.products.length} entradas · ${pgdd.length} com nome+preço (${pgdd.filter(p => isRealEan(p.ean)).length} c/ EAN mpn)`);
  console.log(`📦 Seed actual:        ${seed.products.length} produtos, ${seed.stores.length} lojas · fuzzy_min=${FUZZY_MIN}\n`);

  pgdd = pgdd.slice(0, MAX_PRODUCTS);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    seed.stores.push({
      slug: STORE_SLUG,
      name: (def && def.nome) || 'Pharma GDD',
      base_url: (def && def.url) || 'https://www.pharma-gdd.pt',
    ...
    console.log(`🏬 Loja "Pharma GDD" registada em seed.stores[].`);
  }
  ...
  for (const ep of pgdd) {
    ...
    // Se o produto-alvo tinha EAN sintético e a pgdd traz EAN real (mpn), faz upgrade.
    ...
    const r = upsertStoreItem(
      ...
      target.ean, ep, pgddData.scraped_at
    );
    if (r.action === 'added') added++; else if (r.action === 'merged') updated++;
  }

  console.log('══════ Resumo da integração (pharma-gdd) ══════');
  ...
  console.log(`  Ofertas pharma-gdd:  +${added} adicionadas, ${updated} actualizadas (total ${sp.items.length})`);
  if (fuzzySamples.length) {
    ...
    for (const s of toShow) { console.log(`    [${s.j.toFixed(2)}] pgdd: ${s.a}`); console.log(`          seed: ${s.s}`); }
  }
