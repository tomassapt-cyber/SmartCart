#!/usr/bin/env node
/**
 * CosMath — Integrate pharmavida.pt catalog into seed
 * ============================================================
 * Pharmavida expõe EAN real (mpn) E CNP (sku) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-pharmavida-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'pharmavida-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'pharmavida';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-pharmavida-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 pharmavida: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Pharmavida',
      base_url: 'https://www.pharmavida (ver header)', logo_url: null,
      free_shipping_threshold: 39,
      shipping_zones: { mainland: 3.99, madeira: 3.99, acores: 3.99 },
    });
    console.log('🏬 Loja "Pharmavida" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('pharmavida-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'pharmavida-catalog' };
  ...
  console.log(`  Ofertas pharmavida:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração pharmavida completa.');
})();
