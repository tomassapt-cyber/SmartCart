#!/usr/bin/env node
/**
 * CosMath — Integrate Poupafarma catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/poupafarma-full.json (Jumpseller; JSON-LD sku=CNP/EAN)
 * e funde no seed em MODO COMPARAÇÃO: Poupafarma entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-poupafarma-catalog.js
 *   node scripts/integrate-poupafarma-catalog.js --dry-run
 *   node scripts/integrate-poupafarma-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'poupafarma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'poupafarma';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-poupafarma-catalog.js');
    process.exit(1);
  }
  ...
  const allOk = feed.products.filter(p => p.status === 'ok' && p.price > 0);
  let items = allOk.filter(p => !isNonCosmetic(p.name));
  const droppedNonCosm = allOk.length - items.length;
  console.log(`📦 poupafarma: ${feed.products.length} entradas · ${items.length} cosmética c/ preço (${droppedNonCosm} não-cosmética filtrada: medicamentos/suplem/fórmula/pet)`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo poupafarma. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(poupafarma|sequeira)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'Poupafarma',
      base_url: 'https://www.poupafarma.pt',
      logo_url: null,
  ...
  console.log('══════ Resumo (Poupafarma · fonte de preço, enrich-only) ══════');
  ...
  if (r.status === 0) console.log('\n✅ Integração Poupafarma completa.');
})();
