#!/usr/bin/env node
/**
 * CosMath — Integrate Powerbeauty catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/powerbeauty-full.json (PrestaShop; JSON-LD gtin13=EAN real
 * + sku/mpn=CNP — as duas chaves)
 * e funde no seed em MODO COMPARAÇÃO: Power Beauty entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-powerbeauty-catalog.js
 *   node scripts/integrate-powerbeauty-catalog.js --dry-run
 *   node scripts/integrate-powerbeauty-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'powerbeauty-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'powerbeauty';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-powerbeauty-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 powerbeauty: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo powerbeauty. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(powerbeauty)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    ...
    console.log(`🏬 Loja "Powerbeauty" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (Powerbeauty · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas powerbeauty:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  if (r.status === 0) console.log('\n✅ Integração Powerbeauty completa.');
})();
