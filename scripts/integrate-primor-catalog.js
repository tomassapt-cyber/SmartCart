#!/usr/bin/env node
/**
 * CosMath — Integrate Primor catalog into seed
 * ============================================================
 * Primor (perfumaria ES, storefront PT pt.primor.eu, Magento): JSON-LD com mpn=EAN-13 REAL -> matching forte por EAN. Perfil Druni (mass-market barato).
 *
 ...
 *
 * Uso:
 *   node scripts/integrate-primor-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'primor-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'primor';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-primor-catalog.js --dermo'); process.exit(1); }
  ...
  console.log(`📦 primor: ${data.products.length} entradas · ${cc.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Primor',
      base_url: 'https://pt.primor.eu', logo_url: null,
      // TODO confirmar portes/threshold com o user (valores standard publicados)
      free_shipping_threshold: 39,
      shipping_zones: { mainland: 2.95, madeira: 2.95, acores: 2.95 },
    });
    console.log('🏬 Loja "Primor" registada em seed.stores[].');
  }
  ...
  console.log(`  Ofertas primor:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
