#!/usr/bin/env node
/**
 * CosMath — Integrate saudemayor.pt catalog into seed
 * ============================================================
 * Saúde Mayor expõe EAN real (mpn) E CNP (sku) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-saudemayor-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'saudemayor-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'saudemayor';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-saudemayor-catalog.js'); process.exit(1); }
  ...
  let sm = data.products.filter(p => p.status === 'ok' && isRealEan(p.ean) && p.price > 0).slice(0, MAX);
  console.log(`📦 saudemayor: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Saúde Mayor',
      base_url: 'https://www.saudemayor.pt', logo_url: null,
      free_shipping_threshold: 30,
      shipping_zones: { mainland: 3.99, madeira: 5.99, acores: 5.99 },
    });
    console.log('🏬 Loja "Saúde Mayor" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      target = { ean: ep.ean, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'saudemayor-catalog' };
  ...
  console.log(`  Ofertas saudemayor:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração saudemayor completa.');
})();
