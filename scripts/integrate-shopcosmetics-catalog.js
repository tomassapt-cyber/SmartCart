#!/usr/bin/env node
/**
 * CosMath — Integrate Shopcosmetics catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/shopcosmetics-full.json (PrestaShop; JSON-LD gtin13=EAN real
 * + sku/mpn=CNP — as duas chaves)
 * e funde no seed em MODO COMPARAÇÃO: ShopCosmetics entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-shopcosmetics-catalog.js
 *   node scripts/integrate-shopcosmetics-catalog.js --dry-run
 *   node scripts/integrate-shopcosmetics-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'shopcosmetics-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'shopcosmetics';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-shopcosmetics-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 shopcosmetics: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo shopcosmetics. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(shopcosmetics)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'ShopCosmetics',
      base_url: 'https://shopcosmetics.pt',
      logo_url: null,
      // TODO confirmar portes/threshold reais (PrestaShop; ver site)
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.99, madeira: 5.99, acores: 5.99 },
    });
    console.log(`🏬 Loja "Shopcosmetics" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (Shopcosmetics · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas shopcosmetics:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  if (r.status === 0) console.log('\n✅ Integração Shopcosmetics completa.');
})();
