#!/usr/bin/env node
/**
 * CosMath — Integrate smartbeauty catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/smartbeauty-full.json (Jumpseller; JSON-LD sku=CNP/EAN)
 * e funde no seed em MODO COMPARAÇÃO: smartbeauty entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-smartbeauty-catalog.js
 *   node scripts/integrate-smartbeauty-catalog.js --dry-run
 *   node scripts/integrate-smartbeauty-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'smartbeauty-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'smartbeauty';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-smartbeauty-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 smartbeauty: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo smartbeauty. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(smartbeauty)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'Smart Beauty',
      base_url: 'https://www.smartbeauty.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "smartbeauty" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (smartbeauty · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas smartbeauty:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  if (r.status === 0) console.log('\n✅ Integração smartbeauty completa.');
})();
