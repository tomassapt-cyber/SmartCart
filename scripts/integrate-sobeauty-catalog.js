#!/usr/bin/env node
/**
 * CosMath — Integrate SoBeauty catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/sobeauty-full.json (Jumpseller; JSON-LD sku=CNP/EAN)
 * e funde no seed em MODO COMPARAÇÃO: SoBeauty entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-sobeauty-catalog.js
 *   node scripts/integrate-sobeauty-catalog.js --dry-run
 *   node scripts/integrate-sobeauty-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'sobeauty-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'sobeauty';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-sobeauty-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 sobeauty: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo sobeauty. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(sobeauty)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'SoBeauty',
      base_url: 'https://www.sobeauty.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "SoBeauty" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (SoBeauty · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas sobeauty:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
