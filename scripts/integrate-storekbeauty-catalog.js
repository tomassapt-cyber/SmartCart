#!/usr/bin/env node
/**
 * CosMath — Integrate storekbeauty (ver scraper) catalog into seed
 * ============================================================
 * Store K-Beauty expõe EAN real (mpn) E CNP (sku) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-storekbeauty-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'storekbeauty-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'storekbeauty';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-storekbeauty-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 storekbeauty: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Store K-Beauty',
      base_url: '//www.storekbeauty.com', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "Store K-Beauty" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('storekbeauty-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'storekbeauty-catalog' };
  ...
  console.log('══════ Resumo (storekbeauty) ══════');
  ...
  console.log(`  Ofertas storekbeauty:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração storekbeauty completa.');
})();
