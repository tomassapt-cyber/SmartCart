#!/usr/bin/env node
/**
 * CosMath — Integrate Unifarma catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/unifarma-full.json (Shopify products.json; sku=CNP/EAN)
 * e funde no seed em MODO COMPARAÇÃO: Unifarma entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-unifarma-catalog.js
 *   node scripts/integrate-unifarma-catalog.js --dry-run
 *   node scripts/integrate-unifarma-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'unifarma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'unifarma';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-unifarma-catalog.js');
    process.exit(1);
  }
  ...
  // apply-cnp-merge). Exclui o próprio catálogo unifarma. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(unifarma|needs|yneeds)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'Unifarma',
      base_url: 'https://www.unifarma.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "Unifarma" registada em seed.stores[].`);
  }

  let cnpMatched = 0, matched = 0, looseMatched = 0, fuzzyMatched = 0, noBrand = 0, noMatch = 0, volSkip = 0, cnpBrandSkip = 0, added = 0, updated = 0, imgFilled = 0;
  ...
  console.log('══════ Resumo (Unifarma · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas unifarma:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
