#!/usr/bin/env node
/**
 * CosMath — Integrate vallispharma (ver scraper) catalog into seed
 * ============================================================
 * VallisPharma expõe EAN (bar_code) + CNP (ref) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-vallispharma-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'vallispharma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'vallispharma';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-vallispharma-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 vallispharma: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'VallisPharma',
      base_url: 'https://www.vallispharma.pt', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "VallisPharma" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('vallispharma-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'vallispharma-catalog' };
  ...
  console.log('══════ Resumo (vallispharma) ══════');
  ...
  console.log(`  Ofertas vallispharma:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
  ...
  console.log('\n✅ Integração vallispharma completa.');
})();
