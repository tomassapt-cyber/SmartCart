#!/usr/bin/env node
/**
 * CosMath — Integrate vidamais (ver scraper) catalog into seed
 * ============================================================
 * Farmácia Vida+Mais expõe EAN real (mpn) E CNP (sku) — chave dupla. Matching FORTE
 ...
 *
 * Uso:
 *   node scripts/integrate-vidamais-catalog.js [--dry-run] [--max=N]
 */
...
const FULL = path.join(ROOT, 'data', 'catalog', 'vidamais-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'vidamais';
...
(function main() {
  if (!fs.existsSync(FULL)) { console.error('✗ Não existe', FULL, '\n  Corre: node scripts/scrape-vidamais-catalog.js'); process.exit(1); }
  ...
  console.log(`📦 vidamais: ${data.products.length} entradas · ${sm.length} com EAN real + preço`);
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG, name: 'Farmácia Vida+Mais',
      base_url: 'https://www.farmaciavidamais.pt', logo_url: null,
      free_shipping_threshold: 49,
      shipping_zones: { mainland: 3.95, madeira: 3.95, acores: 3.95 },
    });
    console.log('🏬 Loja "Farmácia Vida+Mais" registada em seed.stores[].');
  }
  ...
  for (const ep of sm) {
    ...
    if (!target) {
      ...
      const newEan = isRealEan(ep.ean) ? ep.ean : ('vidamais-' + (ep.url || '').split('/').pop().replace(/\.html$/, '').slice(0, 50).replace(/[^a-z0-9-]/gi, ''));
      target = { ean: newEan, name: ep.name, brand: displayBrand(ep.brand) || ep.brand || null, category: dermo, image_url: ep.image_url || null, _source: 'vidamais-catalog' };
  ...
  console.log(`  Ofertas vidamais:        +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
