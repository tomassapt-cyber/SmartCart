#!/usr/bin/env node
/**
 * CosMath — Integrate WOWFARMA catalog into seed (FONTE DE PREÇO — nunca cria)
 * ===================================================================
 *
 * Lê data/catalog/wowfarma-full.json (feed KK; reference=CNP + ean=EAN-13)
 * e funde no seed em MODO COMPARAÇÃO: WOWFARMA entra SÓ como fonte de preço
 ...
 *
 * Uso:
 *   node scripts/integrate-wowfarma-catalog.js
 *   node scripts/integrate-wowfarma-catalog.js --dry-run
 *   node scripts/integrate-wowfarma-catalog.js --no-inject
 */
...
const FEED_FULL = path.join(ROOT, 'data', 'catalog', 'wowfarma-full.json');
const SEED_BUNDLE = path.join(ROOT, 'data', 'seed-bundle.json');
const STORE_SLUG = 'wowfarma';
...
(function main() {
  if (!fs.existsSync(FEED_FULL)) {
    console.error('✗ Não existe', FEED_FULL, '\n  Corre primeiro: node scripts/scrape-wowfarma-catalog.js');
    process.exit(1);
  }
  ...
  console.log(`📦 wowfarma: ${feed.products.length} entradas · ${items.length} com preço`);
  ...
  // apply-cnp-merge). Exclui o próprio catálogo wowfarma. É o sinal forte:
  ...
  function resolveBrand(item) {
    ...
    if (v && !/^(wowfarma)/i.test(v)) return v;
  ...
  if (!seed.stores.some(s => s.slug === STORE_SLUG)) {
    seed.stores.push({
      slug: STORE_SLUG,
      name: 'WOWFARMA',
      base_url: 'https://www.wowfarma.pt',
      logo_url: null,
    ...
    console.log(`🏬 Loja "WOWFARMA" registada em seed.stores[].`);
  }
  ...
  console.log('══════ Resumo (WOWFARMA · fonte de preço, enrich-only) ══════');
  ...
  console.log(`  Ofertas wowfarma:      +${added} novas, ${updated} actualizadas (total ${sp.items.length})`);
