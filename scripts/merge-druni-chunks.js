#!/usr/bin/env node
/**
 * SmartCart — Merge Druni chunk files into druni-full.json
 * ============================================================
 *
 * Lê todos os druni-chunk-*.json de data/catalog/ e faz merge
 * num único druni-full.json, deduplicando por URL (mais recente ganha).
 *
 * Uso:
 *   node scripts/merge-druni-chunks.js
 */
...
const chunkFiles = fs.readdirSync(CATALOG_DIR)
  .filter(f => /^druni-chunk-\d+-\d+\.json$/.test(f))
  .map(f => path.join(CATALOG_DIR, f))
  .sort();

if (chunkFiles.length === 0) {
  console.error('✗ Nenhum druni-chunk-*.json encontrado em', CATALOG_DIR);
  process.exit(1);
}
