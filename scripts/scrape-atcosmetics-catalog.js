#!/usr/bin/env node
/**
 * CosMath — ATCosmetics catalog scrape (atcosmetics.pt, Shopify PT)
 * ============================================================
 *
 * ATCosmetics (atcosmetics.pt, ~900 produtos cosmética). Shopify PT, envia PT.
 *
 * DESCOBERTA-CHAVE (scouting 2026-07-16): o `sku` é código interno (6 díg,
 * NÃO é CNP), mas o **EAN-13 real vive no NOME DO FICHEIRO da imagem**:
 * `4250338465781_01.jpg` → validado por checksum GTIN. products.json paginado
 * dá título+vendor+preço+imagens num só fetch por página — zero fichas.
 *
 * Guardamos:
 ...
 *
 * Uso:
 *   node scripts/scrape-atcosmetics-catalog.js              # tudo
 *   node scripts/scrape-atcosmetics-catalog.js --resume     # continua
 *   node scripts/scrape-atcosmetics-catalog.js --limit=250  # smoke test
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'atcosmetics-full.json');
const BASE = 'https://www.atcosmetics.pt';
...
// ATCosmetics: o EAN-13 real é o nome do ficheiro da imagem ({EAN}_01.jpg).
function eanFromImages(p) {
  for (const img of (p.images || [])) {
    const m = String(img.src || '').match(/\/(\d{13})_\d+\.(?:jpg|jpeg|png|webp)/i);
    if (m && gtinOk(m[1]) && !/0{6,}/.test(m[1])) return m[1];
  }
  return null;
}
...
function mapProduct(p, category) {
  ...
  // Chave primária: EAN-13 do nome do ficheiro da imagem (validado por
  // checksum); o sku aqui é interno (6 díg) — só serve de fallback raro.
  let ean = eanFromImages(p), cnp = null;
...
function writeCatalog(byHandle, nextPage) {
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'atcosmetics.pt (Shopify products.json; sku=CNP/EAN)', in_progress: nextPage != null, next_page: nextPage, products }), 'utf8');
}
...
(async function main() {
  ...
  console.log('📦 A buscar catálogo ATCosmetics (Shopify /products.json)…\n');
