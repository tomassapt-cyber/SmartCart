#!/usr/bin/env node
/**
 * GirlMath — Atida (Mifarma PT) catalog scrape
 * ============================================================
 *
 * Atida é Magento mas expõe Product JSON-LD limpo em cada PDP. Não
 ...
 *
 * Uso:
 *   node scripts/scrape-atida-catalog.js              # full
 *   node scripts/scrape-atida-catalog.js --limit=100  # smoke test
 *   node scripts/scrape-atida-catalog.js --chunk=1/4  # 1ª fatia de 4
 *   node scripts/scrape-atida-catalog.js --resume     # retoma de checkpoint
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'atida-full.json');
const SITEMAP_URL = 'https://www.atida.com/media/sitemap/pt/sitemap-products.xml';
...
function extractProductData(html, url) {
  ...
  return {
    ...
    variants: [], // Atida usa páginas separadas por volume → variantes não estão num único PDP
  };
}
...
(async function main() {
  ...
  // 1. Sitemap
  console.log('📋 A descarregar sitemap Atida…');
  ...
  const allUrls = (smXml.match(/<loc>([^<]+)<\/loc>/g) || [])
    ...
    .filter(u => u.startsWith('https://www.atida.com/pt-pt/'));
