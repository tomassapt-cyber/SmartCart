#!/usr/bin/env node
/**
 * CosMath — beleza37.pt catalog scrape (WooCommerce/WordPress, farmácia dermo PT)
 * ============================================================
 *
 * beleza37 é uma farmácia dermo PT (~990 produtos "Saúde e Beleza" no
 ...
 *
 * Uso:
 *   node scripts/scrape-beleza37-catalog.js
 *   node scripts/scrape-beleza37-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'beleza37-full.json');
const BASE = 'https://www.beleza37.pt';
...
// ⚠ Esta plataforma (beleza37/smartbeauty/beleza37 partilham-na) dá 429 com
...
// ⚠ O servidor da beleza37 CORTA ligações do fetch/undici em IPs de datacenter
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'beleza37.pt (WooCommerce PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap_index.xml beleza37…');
