#!/usr/bin/env node
/**
 * CosMath — farmaciacamelo.pt catalog scrape (DynamoCMS/Selenia; SEM sitemap)
 * ============================================================
 ...
 *
 * Uso: node scripts/scrape-camelo-catalog.js [--limit=N] [--resume]
 */
const fs = require('fs');
...
const OUT_FILE = path.join(CATALOG_DIR, 'camelo-full.json');
const BASE = 'https://www.farmaciacamelo.pt';
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'farmaciacamelo.pt (Dynamo/Selenia; all-products?p=N; data-ref=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }
