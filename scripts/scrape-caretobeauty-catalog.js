#!/usr/bin/env node
/**
 * CosMath — Care to Beauty catalog scrape
 * ============================================================
 * Loja portuguesa (Porto, desde 2015). 28.449 produtos no mapa do site PT —
 * a maior loja de cosmética PT que faltava. Magento, tal como a caretobeauty, e com
 * JSON-LD limpo em cada ficha: fetch simples + parse, sem browser.
 *
 * VERIFICADO numa ficha real antes de escrever isto (2026-07-30):
 *   · JSON-LD Product com **gtin** (EAN-13 real) → cruzamento forte por EAN;
 *   · **sku de 7 dígitos = CNP**, portanto o apply-cnp-merge também a apanha;
 *   · price + priceCurrency "EUR", availability, brand.name;
 *   · shippingDetails com addressCountry "PT" — confirma que /pt/ é a loja
 *     portuguesa e não outra moeda.
 *
 * ⚠️ TRÊS DIFERENÇAS face à caretobeauty, todas medidas (copiar sem as ver dava dados
 * errados):
 *   1. O bloco JSON-LD é um **ARRAY** de 5 objectos (Product, FAQPage,
 *      Organization, WebPage, Brand). O extractor da caretobeauty só trata objecto
 *      solto e @graph — sem tratar array, NÃO ENCONTRAVA PRODUTO NENHUM.
 *   2. O campo "image" é um **array de ImageObject**, não de strings. A da
 *      caretobeauty (image[0]) devolveria "[object Object]" como endereço da imagem.
 *   3. O mapa do site é **.gz** e há um por país. Tem de ser o de /pt/ e tem
 *      de ser descomprimido — a caretobeauty lê XML simples.
 *
 * ⚠️ NÃO usar o campo "size" do JSON-LD para o volume: na ficha verificada diz
 * "20ml" e o produto é de 15ml. O volume vem do NOME, como nas outras lojas.
 *
 * Pipeline:
 *  1. Descarregar /pt/sitemaps/products_1.xml.gz (~28k URLs) e descomprimir
 *  2. Filtrar por keywords (evita gastar pedidos em suplementos, etc.)
 *  3. Por URL: fetch + JSON-LD Product + preço anterior do DOM (old-price)
 *  4. Checkpoint a cada 100 produtos (resume seguro)
 *
 * Uso:
 *   node scripts/scrape-caretobeauty-catalog.js              # completo
 *   node scripts/scrape-caretobeauty-catalog.js --limit=100  # ensaio
 *   node scripts/scrape-caretobeauty-catalog.js --chunk=1/4  # 1ª fatia de 4
 *   node scripts/scrape-caretobeauty-catalog.js --resume     # retoma
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'caretobeauty-full.json');
const SITEMAP_URL = 'https://www.caretobeauty.com/pt/sitemaps/products_1.xml.gz';
const URL_PREFIX = 'https://www.caretobeauty.com/pt/';
...
function saveCheckpoint(products, inProgress = true) {
  ...
  const out = {
    ...
    source: 'caretobeauty.com (HTTP + JSON-LD)',
...
function extractProductData(html, url) {
  ...
  for (const m of ldMatches) {
    try {
      ...
      // ARRAY — é assim que a Care to Beauty publica (Product + FAQPage +
      // Organization + WebPage + Brand no mesmo bloco). Sem este ramo, o
      // extractor não encontrava produto nenhum.
      if (Array.isArray(obj)) {
        const p = obj.find(x => x && x['@type'] === 'Product');
  ...
  // 2b. AggregateOffer (marketplace multi-vendedor): ~5.6k fichas caretobeauty usam
  ...
  // 4. Imagem
  // O campo "image" é um array de ImageObject (não de strings) — a fórmula
  // da atida devolvia "[object Object]" como endereço da imagem.
  const primeiraImg = Array.isArray(product.image) ? product.image[0] : product.image;
  const image_url = typeof primeiraImg === 'string' ? primeiraImg : (primeiraImg?.url || null);
  ...
  // sku de 7 dígitos = CNP (Código Nacional do Produto). Guardá-lo faz com
  // que o apply-cnp-merge.js consiga juntar este produto aos das farmácias
  // portuguesas que só publicam CNP.
  ...
  return {
    ...
    variants: [], // Care to Beauty usa páginas separadas por volume → variantes não estão num único PDP
  };
}
...
(async function main() {
  ...
  // 1. Sitemap
  console.log('📋 A descarregar mapa do site Care to Beauty…');
  const smRes = await fetch(SITEMAP_URL, { headers: { 'User-Agent': 'GirlMath-Catalog-Bot/1.0' } });
  // o mapa vem comprimido (.gz) — a atida serve XML simples
  const smBuf = Buffer.from(await smRes.arrayBuffer());
  let smXml;
  try {
    smXml = require('zlib').gunzipSync(smBuf).toString('utf8');
  } catch {
    smXml = smBuf.toString('utf8');   // se um dia servirem sem comprimir
  }
  ...
  async function worker() {
    while (idx < queue.length) {
      ...
      if (r.status === 'ok') {
        ...
        // Soft-block anti-datacenter (classe perfumesclub, 2026-07-10): a caretobeauty
