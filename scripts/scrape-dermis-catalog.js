#!/usr/bin/env node
/**
 * CosMath — dermis.pt catalog scrape (API REST pública)
 * ============================================================
 * DERMIS = farmácia/dermocosmética PT na MESMA plataforma "Ycommerce"/Nuxt do
 * bemecare (mesmo dono; koreanbeautyshopeu.com também redireciona para cá).
 * Descoberta por sniff de browser 2026-07-21: a app pagina contra
 * **https://api.dermis.pt/api/products?take=N&skip=M** — API pública que
 * devolve {list, total, total_without_filters}: sku(=CNP 7 díg, 100%), ean
 * (EAN-13 real, ~81% — muito melhor que o bemecare), name/slug (pt),
 * is_active e combinations[0] com price / campaign_price / system_price /
 * quantity. take=250 aceite → ~18 pedidos para 4.334 produtos. Ficha:
 * dermis.pt/pt/artigo/<slug> (as fichas dão 404 a fetch simples — SSR só via
 * a API; por isso NUNCA visitar fichas, só a API).
 *
 ...
 *
 * Uso: node scripts/scrape-dermis-catalog.js [--limit=N] [--take=250]
 */
const fs = require('fs');
...
const OUT_FILE = path.join(CATALOG_DIR, 'dermis-full.json');
const API = 'https://api.dermis.pt/api/products';
const SITE = 'https://dermis.pt';
...
async function main() {
  ...
  console.log(`📦 DERMIS — API pública (take=${TAKE})…`);
  ...
  if (LIMIT === Infinity) {
    fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'api.dermis.pt/api/products (API pública; sku=CNP, campaign_price)', in_progress: false, products }), 'utf8');
  }
  console.log(`\n══════ dermis scrape ══════`);
