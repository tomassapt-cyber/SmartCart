#!/usr/bin/env node
/**
 * GirlMath — Easyfarma catalog scrape
 * ============================================================
 *
 * Easyfarma é uma loja Shopify. Em vez de browser-automation,
 ...
 *
 * Output: data/catalog/easyfarma-full.json
 * Formato compatível com `integrate-easyfarma-catalog.js`:
 *   {
 ...
 *
 * Uso:
 *   node scripts/scrape-easyfarma-catalog.js              # tudo (~30s)
 *   node scripts/scrape-easyfarma-catalog.js --limit=50   # smoke test
 *   node scripts/scrape-easyfarma-catalog.js --raw        # guarda também o raw
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'easyfarma-full.json');
const RAW_FILE = path.join(CATALOG_DIR, 'easyfarma-raw.json');
...
const BASE = 'https://easyfarma.pt';

// ── Categorização ─────────────────────────────────────────────
// Mapeia o product_type da loja para a nossa taxonomy.
// product_type da Easyfarma é específico (ex: "Hidratantes e Matificantes")
...
(async function main() {
  ...
  console.log('📦 A buscar catálogo Easyfarma (Shopify /products.json)…\n');
