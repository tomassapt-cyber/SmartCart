#!/usr/bin/env node
/**
 * CosMath — farmaciadocostume.pt catalog scrape (Laravel/Livewire, PT)
 * ============================================================
 * Farmácia do Costume: JSON-LD limpo na ficha com **mpn=EAN-13 real** +
 * sku=CNP(7) — chave dupla. Sitemap /sitemap.xml flat (~7.5k) com fichas
 * como slugs de RAIZ (…/<slug>[.html]) misturadas com páginas institucionais
 * (/lojas, /marcas, …) — filtro por denylist + slug não-cosmético; fichas sem
 * JSON-LD Product caem no skip. Imagem boa em /images/originals/{CNP}.webp
 * (o JSON-LD traz um thumbnail cdn-cgi de 60px).
 *
...
const OUT_FILE = path.join(CATALOG_DIR, 'farmaciadocostume-full.json');
const BASE = 'https://www.farmaciadocostume.pt';
...
function isProductUrl(u) {
  // fichas reais terminam SEMPRE .html; slugs de raiz sem .html são páginas institucionais
  const m = u.match(/^https:\/\/(?:www\.)?farmaciadocostume\.pt\/([a-z0-9-]+)\.html$/);
  if (!m) return false;
  return m[1].includes('-') && !NON_PRODUCT.has(m[1]);
}
function slugLooksCosmetic(u) { const slug = ((u.split('/').pop() || '').replace(/\.html$/, '')).replace(/-/g, ' '); return !isNonCosmetic(slug); }
...
function extractProductData(html) {
  ...
  for (const b of blocks) {
    ...
    for (const n of nodes) {
      ...
      const gtin = [n.gtin13, n.gtin, n.gtin14, n.mpn, offer && offer.gtin13].map(x => (x != null ? String(x).trim() : '')).find(x => /^\d{12,14}$/.test(x) && !/0{6,}/.test(x)) || null;
      const skuRaw = [n.sku, offer && offer.sku].map(x => (x != null ? String(x).trim() : '')).find(x => /^\d{7}$/.test(x)) || null;
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'farmaciadocostume.pt (Laravel/Livewire; JSON-LD mpn=EAN + sku=CNP)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap.xml farmaciadocostume…');
