#!/usr/bin/env node
/**
 * CosMath — farmaciagarcia.com catalog scrape (Shopify — Farmácia da Penha, Faro)
 * ============================================================
 ...
 *
 * Uso: node scripts/scrape-farmaciagarcia-catalog.js [--limit=N] [--resume]
 */
const fs = require('fs');
...
const OUT_FILE = path.join(CATALOG_DIR, 'farmaciagarcia-full.json');
const BASE = 'https://loja.farmaciagarcia.net';   // sem www
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'farmaciagarcia.com (Shopify; products.json sku=CNP + <handle>.js barcode=EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A paginar products.json farmaciagarcia…');
  let products = [];
