#!/usr/bin/env node
/**
 * CosMath — farmaciaideal.pt catalog scrape (farmácia PT, EAN+CNP — plataforma saudemayor)
 * ============================================================
 *
 * Farmácia Ideal é uma farmácia PT (MESMA plataforma SaaS da Saúde Mayor) (preços EUR, envia PT). Cada
 ...
 *
 * Uso:
 *   node scripts/scrape-farmaciaideal-catalog.js
 *   node scripts/scrape-farmaciaideal-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'farmaciaideal-full.json');
const BASE = 'https://www.farmaciaideal.pt';
...
function extractProductData(html) {
  ...
  // Diferença vs saudemayor: aceitar EAN OU CNP — os CNP-only entram como
  // sintéticos no integrador e o apply-cnp-merge resgata-os via CNP partilhado.
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'farmaciaideal.pt (plataforma saudemayor: mpn=EAN, sku=CNP; regex sobre JSON-LD)', in_progress: inProgress, products }), 'utf8'); }
