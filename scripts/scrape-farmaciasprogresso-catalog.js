#!/usr/bin/env node
/**
 * CosMath — farmaciasprogresso.pt catalog scrape (PrestaShop, PT)
 * ============================================================
 * Farmácias Progresso (sonda 2026-07-23): JSON-LD na ficha com sku=CNP(7) OU
 * sku=EAN-13, e mpn às vezes EAN-13 (o Avène trazia ambos) — chave dupla
 * flexível. Sitemap /sitemap.xml flat (~5.9k locs) com fichas como slugs de
 * raiz SEM .html misturadas com categorias/institucionais — o filtro real é o
 * JSON-LD Product (páginas sem Product caem no skip). ~2.3k fichas estimadas.
 *
...
const OUT_FILE = path.join(CATALOG_DIR, 'farmaciasprogresso-full.json');
const BASE = 'https://www.farmaciasprogresso.pt';
...
const NON_PRODUCT = new Set(['lojas', 'marcas', 'contactos', 'sobre-nos', 'quem-somos', 'blog', 'campanhas', 'novidades', 'promocoes', 'termos-e-condicoes', 'politica-de-privacidade', 'politica-de-cookies', 'livro-de-reclamacoes', 'checkout', 'carrinho', 'login', 'registo', 'conta', 'faq', 'entregas', 'devolucoes', 'servicos']);

function locs(xml) { return (xml.match(/<loc>([^<]+)<\/loc>/g) || []).map(m => m.replace(/<\/?loc>/g, '').trim().replace(/&amp;/g, '&')); }
function isProductUrl(u) {
  // Presta: fichas são slugs de RAIZ SEM .html (ex.: /avene-reflexe-creme-…-30ml);
  // categorias também têm esse formato — o filtro final é o JSON-LD Product
  // (páginas sem Product caem no skip, padrão costume).
  const m = u.match(new RegExp('^https://(?:www\\.)?farmaciasprogresso\\.pt/([a-z0-9-]+)/?$'));
  if (!m) return false;
  return m[1].includes('-') && !NON_PRODUCT.has(m[1]);
}
function slugLooksCosmetic(u) { const slug = (u.replace(/\/$/, '').split('/').pop() || '').replace(/-/g, ' '); return !isNonCosmetic(slug); }
...
function extractProductData(html) {
  ...
  for (const b of blocks) {
    ...
    for (const n of nodes) {
      ...
      const gtin = [n.gtin13, n.gtin, n.gtin14, n.mpn, n.sku, offer && offer.gtin13].map(x => (x != null ? String(x).trim() : '')).find(x => /^\d{12,14}$/.test(x) && !/0{6,}/.test(x)) || null;
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'farmaciasprogresso.pt (Laravel/Livewire; JSON-LD mpn=EAN + sku=CNP)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap.xml farmaciasprogresso…');
