#!/usr/bin/env node
/**
 * CosMath — fastpharma.pt catalog scrape (Magento, farmácia dermo PT)
 * ============================================================
 *
 * FastPharma é uma farmácia dermo PT (~1295 produtos "Saúde e Beleza" no
 * KuantoKusta). Magento, envia PT. Cada ficha tem JSON-LD com `sku`=`mpn`=CNP
 * (7 díg), marca aninhada, preço, imagem. SEM products.json → ficha-a-ficha.
 *
 * MODO COMPARAÇÃO (enrich-only): match por CNP contra produtos existentes.
 *
 * Sitemap: /sitemap/sitemap_web.xml (flat, ~6278 URLs; 5170 produtos).
 * URLs de produto: /pt-pt/product/<slug>. Domínio sem www (robots).
 *
 * Uso:
 *   node scripts/scrape-fastpharma-catalog.js
 *   node scripts/scrape-fastpharma-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'fastpharma-full.json');
const BASE = 'https://fastpharma.pt';             // sem www (robots)
const SITEMAP_INDEX = BASE + '/sitemap/sitemap_web.xml';
...
const isProductSitemap = u => /_sitemap\.xml$|sitemap_web/i.test(u);
// Magento fastpharma: fichas em /pt-pt/product/<slug>.
function isProductUrl(u) { return /\/pt-pt\/product\/[^/?]+\/?$/i.test(u); }
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'fastpharma.pt (WooCommerce PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap_index.xml fastpharma…');
