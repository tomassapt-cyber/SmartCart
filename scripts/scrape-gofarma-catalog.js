#!/usr/bin/env node
/**
 * CosMath — go-farma.com catalog scrape (Magento, parafarmácia dermo PT)
 * ============================================================
 *
 * GO-farma (Fórmula Maiúscula Lda, Braga) é uma parafarmácia dermo PT (~1038
 * produtos "Saúde e Beleza" no KK). Magento, envia PT. Cada ficha tem JSON-LD
 * com `gtin13`=EAN real nalgumas E `sku`=CNP (7 díg) noutras — chave mista.
 *
 * MODO COMPARAÇÃO (enrich-only): match por EAN+CNP contra produtos existentes.
 *
 * Sitemap: /sitemap.xml (índice) → /sitemaps/sitemap_products_pt.xml (1767
 * URLs). Fichas: /pt/<cat>/<marca>/<slug>_p<id>.html.
 *
 * Uso:
 *   node scripts/scrape-gofarma-catalog.js
 *   node scripts/scrape-gofarma-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'gofarma-full.json');
const BASE = 'https://www.go-farma.com';
...
const isProductSitemap = u => /sitemap_products_pt/i.test(u);
// go-farma: fichas /pt/<cat>/<marca>/<slug>_p<id>.html
function isProductUrl(u) { return /_p\d+\.html$/i.test(u); }
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'go-farma.com (WooCommerce PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap_index.xml gofarma…');
