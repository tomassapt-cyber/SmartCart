#!/usr/bin/env node
/**
 * CosMath — Hiper Farma catalog scrape (hiperfarma.pt, Shopify PT)
 * ============================================================
 *
 * Hiper Farma é uma farmácia dermo PT (~3070 produtos em "Saúde e Beleza" no
 * KuantoKusta). Shopify PT, envia PT. Clone do padrão unifarma (products.json sku=CNP).
 *
 ...
 *
 * Uso:
 *   node scripts/scrape-hiperfarma-catalog.js              # tudo
 *   node scripts/scrape-hiperfarma-catalog.js --resume     # continua
 *   node scripts/scrape-hiperfarma-catalog.js --limit=250  # smoke test
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'hiperfarma-full.json');
const BASE = 'https://www.hiperfarma.pt';
...
function writeCatalog(byHandle, nextPage) {
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'hiperfarma.pt (Shopify products.json; sku=CNP/EAN)', in_progress: nextPage != null, next_page: nextPage, products }), 'utf8');
}
...
(async function main() {
  ...
  console.log('📦 A buscar catálogo Hiper Farma (Shopify /products.json)…\n');
