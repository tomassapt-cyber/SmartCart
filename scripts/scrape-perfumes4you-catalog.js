#!/usr/bin/env node
/**
 * CosMath — perfumes4you.pt catalog scrape (WooCommerce/WordPress, farmácia dermo PT)
 * ============================================================
 *
 * Perfumes4you é uma farmácia dermo PT (~990 produtos "Saúde e Beleza" no
 ...
 *
 * Uso:
 *   node scripts/scrape-perfumes4you-catalog.js
 *   node scripts/scrape-perfumes4you-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'perfumes4you-full.json');
const BASE = 'https://perfumes4you.pt';
...
const isProductSitemap = u => /product-sitemap|product_sitemap|produto-sitemap/i.test(u);
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'perfumes4you.pt (WooCommerce PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap_index.xml perfumes4you…');
