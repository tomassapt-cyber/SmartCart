#!/usr/bin/env node
/**
 * CosMath — pharmascalabis.com catalog scrape (WooCommerce, farmácia PT — CNP no sku)
 * ============================================================
 *
 ...
 *
 * Uso:
 *   node scripts/scrape-pharmascalabis-catalog.js
 *   node scripts/scrape-pharmascalabis-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'pharmascalabis-full.json');
const BASE = 'https://www.pharmascalabis.com';
...
const isProductUrl = u => /\/store\/[^/]+\/?$/.test(u) && !/\/es\//.test(u);
...
function extractProductData(html) {
  ...
  for (const b of blocks) {
    ...
    for (const n of nodes) {
      ...
      const cnp = /^\d{7}$/.test(String(n.sku || '').trim()) ? String(n.sku).trim() : null;
      if (!ean && !cnp) return null;      // precisa de pelo menos uma chave
      ...
      return { name, brand, ean, cnp, sku: n.sku ? String(n.sku) : null, image_url: image_url ? String(image_url).replace(/\\\//g, '/') : null, price, previous_price: null, in_stock, volume_ml: volumeFromName(name), category: null, variants: [] };
    }
  }
  return null;
}
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'pharmascalabis.com (WooCommerce PT; JSON-LD sku=CNP)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap.xml (índice) mycosmetics…');
  const idxXml = await fetchText(SITEMAP_INDEX);
  const sitemapUrls = locs(idxXml).filter(u => /product-sitemap\d*\.xml/.test(u));
