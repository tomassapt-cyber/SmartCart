#!/usr/bin/env node
/**
 * CosMath — pharmeestore.com catalog scrape (Redicom Prolepse, Vila do Conde)
 * ============================================================
 * Pharmee: JSON-LD Product na ficha com gtin13=EAN-13 real + sku/productID=CNP.
 * Sitemap flat /sitemaps/sitemap_products_pt.xml (~3k URLs .._pNNN.html).
 *
...
const OUT_FILE = path.join(CATALOG_DIR, 'pharmee-full.json');
const BASE = 'https://www.pharmeestore.com';
...
function slugLooksCosmetic(u) { const slug = ((u.split('/').pop() || '').replace(/_p\d+\.html$/, '') || '').replace(/-/g, ' '); return slug && !isNonCosmetic(slug); }
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'pharmeestore.com (Redicom; JSON-LD gtin13+sku=CNP)', in_progress: inProgress, products }), 'utf8'); }
