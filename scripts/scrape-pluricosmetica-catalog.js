#!/usr/bin/env node
/**
 * CosMath — pluricosmetica.com catalog scrape (haircare profissional PT, MODO COMPARAÇÃO)
 * ============================================================
 *
 * Maior retalhista PT de cabelo profissional. Plataforma CUSTOM sem gtin/sku/
 * JSON-LD de produto → só nome (h1, com marca em prefixo) + preço
 * (itemprop="price" content=..., PVP cortado em #preco-pub-cut). Por isso:
 * FINGERPRINT exato + guarda de volume, ENRICH-ONLY (integrador nunca cria) —
 * padrão pharma2you/continente. Sitemap /sitemap/produtos (~10k URLs).
 *
 * Uso:
 *   node scripts/scrape-pluricosmetica-catalog.js
 *   node scripts/scrape-pluricosmetica-catalog.js --limit=25   # smoke
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'pluricosmetica-full.json');
const BASE = 'https://www.pluricosmetica.com';
const SITEMAP_URL = BASE + '/sitemap/produtos';
...
const isProductUrl = u => /\/catalog\/\d+\//.test(u);
...
function extractProductData(html) {
  const h1 = (html.match(/<h1[^>]*>([\s\S]{0,200}?)<\/h1>/) || [])[1];
  const name = h1 ? h1.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : null;
  ...
  const prevM = html.match(/id="preco-pub-cut"[^>]*>\s*([\d]+[.,]\d{2})/);
  const previous_price = prevM ? parseFloat(prevM[1].replace(',', '.')) : null;
  const in_stock = /prod-view-btns-quantidades|adicionar/i.test(html) && !/esgotado/i.test(html);
  const img = (html.match(/property="og:image" content="([^"]+)"/) || [])[1] || null;
  // marca fica null — o integrador resolve por prefixo do nome vs marcas do seed
  return { name, brand: null, ean: null, sku: null, image_url: img, price, previous_price: previous_price && previous_price > price ? previous_price : null, in_stock, volume_ml: volumeFromName(name), category: 'hair', variants: [] };
}
...
function saveCheckpoint(products, inProgress = true) {
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'pluricosmetica.com (custom sem gtin; MODO COMPARAÇÃO por fingerprint — nunca cria)', in_progress: inProgress, products }), 'utf8');
}

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap de produtos pluricosmetica…');
