#!/usr/bin/env node
/**
 * CosMath — poupafarma.pt catalog scrape (Jumpseller, farmácia dermo PT)
 * ============================================================
 *
 * Poupafarma (Sequeira Cyrne e Silva, Porto) é uma farmácia dermo grande do
 * KuantoKusta (~2800 produtos "Saúde e Beleza"). Plataforma Jumpseller, envia
 * PT. Cada ficha tem JSON-LD com `sku` = CNP (7 díg) na maioria + EAN-13
 * nalguns, marca, preço, imagem. SEM products.json → scrape ficha-a-ficha.
 *
 * MODO COMPARAÇÃO (enrich-only): match por CNP/EAN contra produtos existentes.
 *
 * Sitemap: /sitemap.xml FLAT (urlset, ~12k URLs). Produtos = slug de topo
 * (/<slug>, 4 segmentos, com hífens); nav/categorias/medicamentos filtrados.
 *
 * Uso:
 *   node scripts/scrape-poupafarma-catalog.js
 *   node scripts/scrape-poupafarma-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'poupafarma-full.json');
const BASE = 'https://www.poupafarma.pt';
...
// poupafarma (Jumpseller): produtos = slug de topo (host/<slug>), com hífens.
// Excluímos páginas de navegação/institucionais e slugs claramente não-produto.
const NAV_SLUG = /^(contact|contactos?|blog|blog-post|medicamentos|medicamentos-com-receita|politica|termos|conta|login|carrinho|marcas|categoria|categorias|sobre|faq|envio|entrega|como-|formas-|devolu|reclama|cookies|ficha|estatuto|newsletter|favoritos|pesquisa|search|pt|en)$/i;
function isProductUrl(u) {
  let p; try { p = new URL(u); } catch { return false; }
  const segs = p.pathname.replace(/^\/+|\/+$/g, '').split('/');
  if (segs.length !== 1) return false;              // só slug de topo
  const slug = segs[0];
  return slug.length > 8 && /-/.test(slug) && !NAV_SLUG.test(slug);
}
// Excluir claramente-não-cosmética pelo NOME da ficha (medicamentos/suplem/pet/bebé).
// Filtro não-cosmética partilhado (lib) — por forma farmacêutica/fórmula/pet,
// nunca por ingrediente (deixa passar "Vitamina C Sérum", "Creme Colagénio").
const EXCLUDE_NAME = { test: isNonCosmetic };
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'poupafarma.pt (Jumpseller PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap.xml (flat) poupafarma…');
