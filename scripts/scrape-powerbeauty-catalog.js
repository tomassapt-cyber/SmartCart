#!/usr/bin/env node
/**
 * CosMath — powerbeauty.pt catalog scrape (PrestaShop, beleza/dermo PT)
 * ============================================================
 *
 * Power Beauty (Superpet lda, Cruz Quebrada) é a MAIOR loja de cosmética do
 * KuantoKusta (19.621 produtos "Saúde e Beleza"). PrestaShop, envia PT.
 * Fichas com JSON-LD **gtin13 = EAN REAL** (sku/mpn = ref interna NE…).
 *
 * MODO COMPARAÇÃO (enrich-only): match por EAN contra produtos existentes.
 *
 * Sitemap: /1_index_sitemap.xml → 1_pt_0_sitemap.xml (~22.6k URLs, CDATA).
 * Fichas: /<categoria>/<id>-<slug> (SEM .html). Domínio SEM www.
 *
 * ⚠️ Catálogo GRANDE (22k fichas ≈ 45-75 min) → padrão DocMorris:
 *   • diário: --known-only (refresca só os URLs das ofertas que já temos no seed)
 *   • semanal: --full --resume (scan completo p/ descobrir novos matches)
 *
 * Uso:
 *   node scripts/scrape-powerbeauty-catalog.js --full          # scan completo
 *   node scripts/scrape-powerbeauty-catalog.js --known-only    # refresh diário
 *   node scripts/scrape-powerbeauty-catalog.js --limit=200     # smoke
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'powerbeauty-full.json');
const BASE = 'https://powerbeauty.pt';          // sem www (o www redireciona)
...
const KNOWN_ONLY = !!args['known-only'];          // diário: só URLs das ofertas do seed
...
function knownUrlsFromSeed() {
  try {
    const seed = JSON.parse(fs.readFileSync(SEED_BUNDLE, 'utf8'));
    const g = (seed.store_products || []).find(x => x.store_slug === 'powerbeauty');
    return [...new Set((g ? g.items : []).map(it => it.url).filter(Boolean))];
  } catch { return []; }
}
...
// PrestaShop: o índice só tem 1 sub-sitemap (1_pt_0_sitemap.xml) — aceitar todos.
const isProductSitemap = u => /_sitemap\.xml$/i.test(u);
// PrestaShop: fichas em /<categoria>/<id>-<slug>.html
function isProductUrl(u) { let p; try { p = new URL(u).pathname; } catch { return false; } return /^\/[^/]+\/\d+-[^/]+$/i.test(p); }
...
function extractProductData(html) {
  ...
  for (const b of blocks) {
    ...
    for (const n of nodes) {
      ...
      // Chaves (Presta): gtin13 = EAN real; sku/mpn = CNP (7 díg).
      const sku = n.sku != null ? String(n.sku).trim() : '';
      const mpn = n.mpn != null ? String(n.mpn).trim() : '';
      ...
      let cnp = /^\d{7}$/.test(sku) ? sku : (/^\d{7}$/.test(mpn) ? mpn : null);
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'powerbeauty.pt (PrestaShop; JSON-LD gtin13=EAN real)' + (KNOWN_ONLY ? ' [known-only]' : ''), in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  let urls = [];
  if (KNOWN_ONLY) {
    urls = knownUrlsFromSeed();
    console.log(`📋 --known-only: ${urls.length} URLs das ofertas powerbeauty no seed`);
    if (!urls.length) { console.error('✗ 0 URLs conhecidas (loja ainda não integrada?) — corre --full primeiro.'); process.exit(1); }
  } else {
    console.log('📋 A descarregar 1_index_sitemap.xml powerbeauty…');
