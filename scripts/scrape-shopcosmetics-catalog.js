#!/usr/bin/env node
/**
 * CosMath — shopcosmetics.pt catalog scrape (PrestaShop, parafarmácia dermo PT)
 * ============================================================
 *
 * Shopcosmetics (Farmácia Brito Martins, Barcelos) é uma parafarmácia dermo PT
 * (~905 produtos "Saúde e Beleza" no KuantoKusta). PrestaShop. Cada ficha tem
 * JSON-LD com **gtin13 REAL + sku/mpn = CNP** — as DUAS chaves (a melhor combinação).
 *
 * MODO COMPARAÇÃO (enrich-only): match por EAN+CNP contra produtos existentes.
 *
 * Sitemap: /1_index_sitemap.xml (índice) → 1_pt_0_sitemap.xml (~1486 URLs).
 * Os <loc> vêm em CDATA → locs() trata disso. Fichas: /<categoria>/<id>-<slug>.html.
 * NB: domínio SEM www (o www redireciona).
 *
 * Uso:
 *   node scripts/scrape-shopcosmetics-catalog.js
 *   node scripts/scrape-shopcosmetics-catalog.js --limit=200 --resume
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'shopcosmetics-full.json');
const BASE = 'https://shopcosmetics.pt';          // sem www (o www redireciona)
...
function isProductUrl(u) { return /\/\d+-[^/]+\.html$/i.test(u); }
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'shopcosmetics.pt (WooCommerce PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap_index.xml shopcosmetics…');
