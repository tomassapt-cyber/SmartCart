#!/usr/bin/env node
/**
 * CosMath — smartbeauty.pt catalog scrape (WooCommerce/WordPress, farmácia dermo PT)
 * ============================================================
 *
 * smartbeauty é uma farmácia dermo PT (~990 produtos "Saúde e Beleza" no
 ...
 *   node scripts/scrape-smartbeauty-catalog.js --limit=200 --resume
 */
...
// ⚠ Esta plataforma (smartbeauty/smartbeauty/beleza37 partilham-na) dá 429 com
...
// ⚠ O servidor da smartbeauty CORTA ligações do fetch/undici em IPs de datacenter
...
function saveCheckpoint(products, inProgress = true) { if (LIMIT !== Infinity) return; /* smoke-test (--limit) NÃO sobrescreve o catálogo de produção */ fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'smartbeauty.pt (WooCommerce PT; JSON-LD sku=CNP/EAN)', in_progress: inProgress, products }), 'utf8'); }

async function main() {
  if (!fs.existsSync(CATALOG_DIR)) fs.mkdirSync(CATALOG_DIR, { recursive: true });
  console.log('📋 A descarregar sitemap_index.xml smartbeauty…');
