#!/usr/bin/env node
/**
 * CosMath — Store K-Beauty catalog scrape (storekbeauty.pt, Shopify PT)
 * ============================================================
 *
 * Store K-Beauty (storekbeauty.com, ~700 produtos K-beauty). Shopify com locale
 * PT dedicado (/pt-pt), envia PT, preços EUR.
 *
 * DESCOBERTA-CHAVE (scouting 2026-07-21): o `/products.json` traz o **sku em
 * formato EAN-13 real** (ex.: 8800296551550), idêntico ao gtin13 do JSON-LD, e
 * o preço já em EUR. vendor = marca coreana. UM fetch paginado, sem visitar
 * fichas. Produtos coreanos importados → NÃO há CNP (código de farmácia PT),
 * mas o EAN-13 chega para casar.
 *
 * Guardamos:
 *   • `ean`  do sku (13 díg) → GTIN real (única chave; sem CNP).
 *
 ...
 *
 * Uso:
 *   node scripts/scrape-storekbeauty-catalog.js              # tudo
 *   node scripts/scrape-storekbeauty-catalog.js --resume     # continua
 *   node scripts/scrape-storekbeauty-catalog.js --limit=250  # smoke test
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'storekbeauty-full.json');
const BASE = 'https://www.storekbeauty.com';
...
function writeCatalog(byHandle, nextPage) {
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'storekbeauty.pt (Shopify products.json; sku=CNP/EAN)', in_progress: nextPage != null, next_page: nextPage, products }), 'utf8');
}
...
(async function main() {
  ...
  console.log('📦 A buscar catálogo Store K-Beauty (Shopify /products.json)…\n');
