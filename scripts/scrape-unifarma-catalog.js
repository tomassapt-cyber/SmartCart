#!/usr/bin/env node
/**
 * CosMath — Unifarma catalog scrape (unifarma.pt, Shopify PT)
 * ============================================================
 *
 * Unifarma é uma farmácia dermo PT (~1936 produtos em "Saúde e Beleza" no
 * KuantoKusta). Shopify PT, envia PT. Mesmo padrão da a-tua-farmacia.
 *
 ...
 *
 * Uso:
 *   node scripts/scrape-unifarma-catalog.js              # tudo
 *   node scripts/scrape-unifarma-catalog.js --resume     # continua
 *   node scripts/scrape-unifarma-catalog.js --limit=250  # smoke test
 */
...
const OUT_FILE = path.join(CATALOG_DIR, 'unifarma-full.json');
const BASE = 'https://www.unifarma.pt';
...
function writeCatalog(byHandle, nextPage) {
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify({ scraped_at: new Date().toISOString(), source: 'unifarma.pt (Shopify products.json; sku=CNP/EAN)', in_progress: nextPage != null, next_page: nextPage, products }), 'utf8');
}
...
(async function main() {
  ...
  console.log('📦 A buscar catálogo Unifarma (Shopify /products.json)…\n');
